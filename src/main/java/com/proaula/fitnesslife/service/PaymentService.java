...
import com.proaula.fitnesslife.controller.AdminPaymentController;
...
public class PaymentService {
    ...
    public List<Payment> getAllPayments() {
        ...
        logger.info("Se encontraron {} pagos", payments.size());
        return payments;
    }
    ...
    public Payment updatePaymentStatus(String id, String newStatus) {
        logger.info("Actualizando estado del pago {} a {}", id, newStatus);
        ...
        if ("ACCEPTED".equals(newStatus) && !"ACCEPTED".equals(oldStatus)) {
            ...
            logger.info("Usuario {} actualizado con plan {}", user.getId(), user.getPlan());
        }

        if (("REJECTED".equals(newStatus) || "CANCELLED".equals(newStatus)) && "ACCEPTED".equals(oldStatus)) {
            User user = payment.getUser();
            user.setPlan(null);
            userService.save(user);
            ...
            logger.info("Plan removido del usuario {}", user.getId());
        }

        Payment updated = paymentRepository.save(payment);
        logger.info("Estado del pago actualizado de {} a {}", oldStatus, newStatus);
        return updated;
    }
    ...
    public void deletePayment(String id) {
        ...
        if ("ACCEPTED".equals(payment.getStatus())) {
            ...
            user.setPlan(null);
            userService.save(user);
            logger.info("Plan removido del usuario {} antes de eliminar el pago", user.getId());
        }
    ...
    public Map<String, Object> getPaymentStatistics() {
        logger.info("Calculando estadísticas de pagos");
        ...
        logger.info("Estadísticas calculadas: {} pagos totales, {} activos", totalPayments, activePayments);
        return stats;
    }

    public List<Payment> getExpiredPayments() {
        logger.info("Buscando pagos expirados");
        LocalDateTime now = LocalDateTime.now();
        return paymentRepository.findExpiredPayments(now);
    }

    public Page<Payment> getPaymentsPaginated(int page, int size, String status, String search) {
        logger.info("Obteniendo pagos paginados - Página: {}, Tamaño: {}, Status: {}, Búsqueda: {}",
        ...
        if (search != null && !search.trim().isEmpty() && status != null && !status.trim().isEmpty()) {
            logger.info("Buscando pagos con estado '{}' y término '{}'", status, search);
            paymentsPage = paymentRepository.searchPaymentsByStatus(status, search, pageable);
        } else if (search != null && !search.trim().isEmpty()) {
            logger.info("Buscando pagos con término '{}'", search);
            paymentsPage = paymentRepository.searchPayments(search, pageable);
        } else if (status != null && !status.trim().isEmpty()) {
            logger.info("Filtrando pagos por estado '{}'", status);
        ...
        logger.info("Se encontraron {} pagos de {} totales",
